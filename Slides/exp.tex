...
\begin{frame}
  ...
  \begin{columns}
    ...
    \draw (0.2,1.25) node {$\textcolor{blue}{m}$};
    \draw (1.9,1.45) node {$\textcolor{red}{M}$};
    ...
    $$\textcolor{red}{M} = \textcolor{blue}{m} + T\textcolor{mygreen}{w}$$
  ...
  \end{columns}
    \vspace{1cm}
    \begin{itemize}
    \item \textcolor{blue}{$m$} : speaker and channel independent supervector
...
Deep neural network are interesting because:
\begin{itemize}
\item<1-> Generates automatically several level of representation
\item<2-> Brings out unsuspected features
\item<3-> There is a multitude of architectures
